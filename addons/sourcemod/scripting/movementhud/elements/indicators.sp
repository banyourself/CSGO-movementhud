...
void OnPlayerRunCmdPost_Element_Indicators(int client, int target)
{
    ...
    // Nothing to show is the usual case, and it used to re-send an empty HudMsg
    // every tick, which both burned bandwidth and kept a HUD channel claimed
    // for an element that was drawing nothing.
    if (!HudNeedsSend(client, MHUD_SLOT_INDICATORS, buffer, xy, rgb))
    {
        return;
    }
