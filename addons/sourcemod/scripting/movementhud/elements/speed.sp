...
void OnPlayerRunCmdPost_Element_Speed(int client, int target)
{
    ...
    // Moving, this changes every tick and still sends every tick. Standing
    // still or spectating a stationary player, it stops sending entirely.
    char buffer[160];
    if (mode == SpeedMode_Float)
    {
        if (!showTakeoff || !gB_DidTakeoff[target])
        {
            FormatEx(buffer, sizeof(buffer), "%.2f", speed);
        }
        else
        {
            FormatEx(buffer, sizeof(buffer), "%.2f\n(%.2f)", speed, gF_TakeoffSpeed[target]);
        }
    }
    else
    {
        ...
        if (!showTakeoff || !gB_DidTakeoff[target])
        {
            FormatEx(buffer, sizeof(buffer), "%d", speedInt);
        }
        else
        {
            FormatEx(buffer, sizeof(buffer), "%d\n(%d)", speedInt, RoundFloat(gF_TakeoffSpeed[target]));
        }
    }

    if (!HudNeedsSend(client, MHUD_SLOT_SPEED, buffer, xy, rgb))
    {
        return;
    }
